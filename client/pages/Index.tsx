...
import { PROFILE_COUNTRIES, PROFILE_INDUSTRIES } from "@/features/leads/constants";
const TOP_UP_PACKS = [
...
export default function Index() {
  ...
  const availableCountries = useMemo(() => {
    ...
    return sorted.length > 0 ? sorted : PROFILE_COUNTRIES;
  }, [leads]);

  const availableIndustries = useMemo(() => {
    const unique = new Set(leads.map((lead) => lead.industry));
    const sorted = Array.from(unique).sort((a, b) => a.localeCompare(b));
    return sorted.length > 0 ? sorted : PROFILE_INDUSTRIES;
  }, [leads]);
  ...
  const handleLoadSeedData = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Unable to load sample leads";
      toast.error(message);
    } finally {
            ...
            <div className="flex flex-col items-center justify-center flex-1 text-center">
              <div className="text-slate-500 dark:text-slate-400 space-y-4">
                <div>
                  ...
                  <p className="text-sm">
                    Import your CSV, load the built-in sample data (4 demo rows), or ask the AI chat to create new prospects.
                  </p>
                </div>
      ...
      <Dialog open={dashboardOpen} onOpenChange={setDashboardOpen}>
        <DialogContent className="dark:bg-slate-900">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 border-t border-slate-200 dark:border-slate-800 pt-4 text-sm">
              <div className="font-semibold">Need quick demo leads?</div>
              <p className="text-slate-500 dark:text-slate-400 text-xs">
                Loads the editable CSV at <code>backend/data/seed_leads.csv</code> (currently 4 rows) into your account.
              </p>
              <Button variant="outline" onClick={handleLoadSeedData} disabled={seedLoading}>
                {seedLoading ? (
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                ) : (
                  <ArrowDown className="h-4 w-4 mr-2" />
